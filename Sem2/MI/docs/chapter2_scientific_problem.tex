...
Low-dose computed tomography (CT) screening has drastically improved the early
detection of lung cancer, but at the same time it introduced a clinical
dilemma: the management of indeterminate lung nodules. When a nodule is
detected, its clinical trajectory is highly uncertain. It may remain harmless
and slow-growing for a patient's entire life, but it could also progress into
aggressive metastatic lung cancer at a fast pace.
...
Deep learning algorithms are uniquely capable of automatically extracting latent hierarchical representations from raw pixel and voxel data. More importantly, a multi-task algorithm is hypothesized to better capture the
biological reality that a nodule's morphology, growth rate, and ultimate malignancy are deeply interdependent phenomena, rather than isolated variables.

\section{Solution Methods}
\label{section:solutionMethods}

To effectively predict nodule malignancy (T1), progression (T2), and subtype (T3) from medical images, we evaluate two different frameworks: an approach that treats each task separately, and a joint-learning method designed to capture their biological overlap. Note that while the full three-task formulation defines the intended scope of this work, T2 (progression) was excluded from the final implementation due to a data linkage constraint described in detail in Section~\ref{section:experimentsAndResults}: the clinical file containing progression labels does not share a common identifier with the DICOM XML annotation files, making it impossible to unambiguously match progression labels to individual CT crops.
...
The objective is to formulate a multi-task learning mapping function capable of joint inference over nodule-level CT imaging data.

\subsection{Inputs}

Let the input space be defined as $X \subset \mathbb{R}^{1 \times 128 \times 128}$, where each element $x \in X$ is a single-channel, two-dimensional CT crop of size $128 \times 128$ pixels centered on a radiologist-annotated nodule region of interest. Raw pixel values are converted to Hounsfield Units (HU) using the DICOM \texttt{RescaleSlope} and \texttt{RescaleIntercept} attributes, clipped to the range $[-1000,\,400]$\,HU, and normalized to $[0, 1]$.
...
\begin{itemize}
  \item \textbf{T1 --- Nodule malignancy risk:} $\hat{Y}_1 \in \{0, 1\}$, where 0 is Benign and 1 is Malignant.
  \item \textbf{T2 --- Two-year progression probability:} $\hat{Y}_2 \in \{0, 1\}$, where 0 is Stable and 1 is Progressive.
  \item \textbf{T3 --- Radiological subtype classification:} $\hat{Y}_3 \in \{0, 1, 2\}$, representing a multiclass prediction (0 = solid, 1 = part-solid, 2 = ground-glass).
\end{itemize}

\subsection{Objective}
The network optimizes task-specific parameters $\theta_1, \theta_2, \theta_3$, potentially alongside a foundational shared backbone $\theta_{\mathrm{shared}}$, and cross-talk routing parameters $\alpha$ (cross-stitch matrices / co-attention weights). These parameters are jointly trained to minimize a globally weighted loss function:
...
where $\mathcal{L}_{BCE}$ is Binary Cross-Entropy, $\mathcal{L}_{CE}$ is Categorical Cross-Entropy, and $\lambda_i$ are dynamic weighting hyperparameters to balance task gradients.
...
From an AI perspective, translating deeply interconnected biological traits into a computational architecture is a state-of-the-art challenge. Designing cross-talk mechanisms that avoid negative transfer in noisy, high-dimensional medical data pushes the boundaries of multi-task network design.