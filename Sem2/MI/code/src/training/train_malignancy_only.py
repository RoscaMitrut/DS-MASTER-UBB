...
DEFAULT_MODEL_OUT = PROJECT_ROOT / "code/src/training/malignancy_only.pt"
...
class LIDCMalignancyFrameDataset(Dataset):
    def __init__(
        self,
        csv_path: Path,
...
class MalignancyOnlyNet(nn.Module):
    def __init__(self, n_malignancy_classes: int = N_MALIGNANCY_CLASSES) -> None:
        super().__init__()
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> dict[str, Any]:
    ...
    with torch.no_grad():
        for images, mal_labels in loader:
            images = images.to(device)
            mal_labels = mal_labels.to(device)

            logits = model(images)
            loss = ce(logits, mal_labels)

            total_loss += float(loss.item())
            preds.append(torch.argmax(logits, dim=1).cpu())
            labels.append(mal_labels.cpu())
            n_batches += 1
    ...
    if n_batches == 0:
        ...
        return {"loss": 0.0, "malignancy": metrics}
    ...
    return {"loss": total_loss / n_batches, "malignancy": metrics}


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train a single-task CT model on LIDC nodule frames for malignancy classification. "
            "Input: 2D CT frame. Output: binary malignancy (0/1)."
        )
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, mal_labels in train_loader:
            images = images.to(device)
            mal_labels = mal_labels.to(device)
            ...
            labels.append(mal_labels.detach().cpu())
            n_batches += 1
        ...
        print(
            ...
            f"{format_metrics('train_mal', train_metrics['malignancy'])} | "
            f"val_loss={val_metrics['loss']:.4f} "
            f"{format_metrics('val_mal', val_metrics['malignancy'])}"
        )
    ...
    print(
        ...
        f"{format_metrics('malignancy', test_metrics['malignancy'])}"
    )
