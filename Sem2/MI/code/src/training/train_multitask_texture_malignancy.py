...
DEFAULT_MODEL_OUT = PROJECT_ROOT / "code/src/training/multitask_texture_malignancy_cross_talk.pt"
...
MALIGNANCY_CLASS_NAMES = ["benign", "malignant"]
N_TEXTURE_CLASSES = 3
N_MALIGNANCY_CLASSES = 2
...
class LIDCNoduleFrameDataset(Dataset):
    def __init__(
        self,
        csv_path: Path,
    ...
    def _build_index(self, max_samples: int | None) -> None:
        ...
        for _, row in df.iterrows():
            ...
            malignancy_binary = parse_int_or_none(row.get("malignancy_binary_majority"))
            texture_index = TEXTURE_CATEGORY_TO_INDEX.get(texture_category)
            if texture_index is None or malignancy_binary is None:
                continue
            if malignancy_binary not in (0, 1):
                continue
            ...
            if bbox is None and bbox_map:
                # Fallback: any bbox from the same nodule if the selected UID has no bbox.
            ...
            self.samples.append(
                {
                    ...
                    "malignancy_label": malignancy_binary,
                }
            )
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        image = resize_2d(image, self.image_size)  # [1,H,W]
        ...
        malignancy = torch.tensor(sample["malignancy_label"], dtype=torch.long)
        return image, texture, malignancy
...
class CrossStitchUnit(nn.Module):
    """
    Soft sharing / cross-talk unit.
    Learns a 2x2 mixing matrix between texture and malignancy feature streams.
    """

    def __init__(self) -> None:
        super().__init__()
        self.mix = nn.Parameter(torch.tensor([[0.9, 0.1], [0.1, 0.9]], dtype=torch.float32))

    def forward(self, tex: torch.Tensor, mal: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        tex_new = self.mix[0, 0] * tex + self.mix[0, 1] * mal
        mal_new = self.mix[1, 0] * tex + self.mix[1, 1] * mal
        return tex_new, mal_new


class CoAttentionUnit(nn.Module):
    """
    Channel-wise co-attention between texture and malignancy streams.
    Each task uses a global descriptor from the other stream to gate the
    cross-task residual information it receives.
    """

    def __init__(self, channels: int, reduction: int = 8) -> None:
        super().__init__()
        hidden = max(4, channels // reduction)
        self.tex_from_mal = nn.Sequential(
            nn.Linear(channels, hidden),
            nn.ReLU(inplace=True),
            nn.Linear(hidden, channels),
            nn.Sigmoid(),
        )
        self.mal_from_tex = nn.Sequential(
            nn.Linear(channels, hidden),
            nn.ReLU(inplace=True),
            nn.Linear(hidden, channels),
            nn.Sigmoid(),
    ...
    def forward(self, tex: torch.Tensor, mal: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        mal_descriptor = mal.mean(dim=(2, 3))
        tex_descriptor = tex.mean(dim=(2, 3))

        tex_gate = self.tex_from_mal(mal_descriptor).unsqueeze(-1).unsqueeze(-1)
        mal_gate = self.mal_from_tex(tex_descriptor).unsqueeze(-1).unsqueeze(-1)

        tex_new = tex + tex_gate * mal
        mal_new = mal + mal_gate * tex
        return tex_new, mal_new


def build_cross_talk_unit(mode: str, channels: int) -> nn.Module:
    if mode == "cross_stitch":
        return CrossStitchUnit()
    if mode == "co_attention":
        return CoAttentionUnit(channels=channels)
    raise ValueError(f"Unsupported cross-talk mode: {mode}")


class MultiTaskCrossTalkNet(nn.Module):
    def __init__(
        self,
        n_texture_classes: int = N_TEXTURE_CLASSES,
        n_malignancy_classes: int = N_MALIGNANCY_CLASSES,
        cross_talk: str = "cross_stitch",
    ) -> None:
        super().__init__()
        channels = [1, 32, 64, 128]
        self.tex_blocks = nn.ModuleList(
            [ConvBlock(channels[i], channels[i + 1]) for i in range(len(channels) - 1)]
        )
        self.mal_blocks = nn.ModuleList(
            [ConvBlock(channels[i], channels[i + 1]) for i in range(len(channels) - 1)]
        )
        self.cross_talk_mode = cross_talk
        self.cross_talk_units = nn.ModuleList(
            [build_cross_talk_unit(cross_talk, channels[i + 1]) for i in range(len(channels) - 1)]
        )
        ...
        self.mal_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.2),
            nn.Linear(64, n_malignancy_classes),
        )

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        tex = x
        mal = x
        for tex_block, mal_block, cross in zip(self.tex_blocks, self.mal_blocks, self.cross_talk_units):
            tex = tex_block(tex)
            mal = mal_block(mal)
            tex, mal = cross(tex, mal)
            tex = self.pool(tex)
            mal = self.pool(mal)

        tex = self.gap(tex)
        mal = self.gap(mal)
        tex_logits = self.tex_head(tex)
        mal_logits = self.mal_head(mal)
        return tex_logits, mal_logits
...
def accuracy_from_logits_onehot_targets(logits: torch.Tensor, onehot_targets: torch.Tensor) -> float:
    preds = torch.argmax(logits, dim=1)
    labels = torch.argmax(onehot_targets, dim=1)
    return float((preds == labels).float().mean().item())


def one_hot_cross_entropy(logits: torch.Tensor, onehot_targets: torch.Tensor) -> torch.Tensor:
    # One-hot CE for mutually exclusive texture classes.
...
def format_task_metrics(prefix: str, metrics: dict[str, Any]) -> str:
    return (
...
def evaluate(
    model: nn.Module,
    loader: DataLoader,
    texture_loss_weight: float,
    malignancy_loss_weight: float,
    device: torch.device,
) -> dict[str, Any]:
    model.eval()
    ce_malignancy = nn.CrossEntropyLoss()
    total_loss = 0.0
    n_batches = 0
    texture_preds: list[torch.Tensor] = []
    texture_labels: list[torch.Tensor] = []
    malignancy_preds: list[torch.Tensor] = []
    malignancy_labels: list[torch.Tensor] = []
    with torch.no_grad():
        for images, tex_onehot, mal_labels in loader:
            images = images.to(device)
            tex_onehot = tex_onehot.to(device)
            mal_labels = mal_labels.to(device)

            tex_logits, mal_logits = model(images)
            tex_loss = one_hot_cross_entropy(tex_logits, tex_onehot)
            mal_loss = ce_malignancy(mal_logits, mal_labels)
            loss = texture_loss_weight * tex_loss + malignancy_loss_weight * mal_loss

            total_loss += float(loss.item())
            texture_preds.append(torch.argmax(tex_logits, dim=1).cpu())
            texture_labels.append(torch.argmax(tex_onehot, dim=1).cpu())
            malignancy_preds.append(torch.argmax(mal_logits, dim=1).cpu())
            malignancy_labels.append(mal_labels.cpu())
            n_batches += 1
    ...
    if n_batches == 0:
        return {
            ...
            "malignancy": compute_classification_metrics(
                preds=torch.empty(0, dtype=torch.long),
                labels=torch.empty(0, dtype=torch.long),
                class_names=MALIGNANCY_CLASS_NAMES,
            ),
        }

    texture_metrics = compute_classification_metrics(
        preds=torch.cat(texture_preds),
        labels=torch.cat(texture_labels),
        class_names=TEXTURE_CLASS_NAMES,
    )
    malignancy_metrics = compute_classification_metrics(
        preds=torch.cat(malignancy_preds),
        labels=torch.cat(malignancy_labels),
        class_names=MALIGNANCY_CLASS_NAMES,
    )
    return {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Train a multi-task soft-sharing/cross-talk model on LIDC nodule frames. "
            "Input: 2D CT frame. Outputs: texture category (3-class one-hot) and binary malignancy (0/1)."
        )
    )
    ...
    parser.add_argument("--texture-loss-weight", type=float, default=1.0)
    parser.add_argument("--malignancy-loss-weight", type=float, default=1.0)
    parser.add_argument(
        "--cross-talk",
        "--co-attention",
        dest="cross_talk",
        choices=["cross_stitch", "co_attention"],
        default="co_attention",
        help="Feature sharing mechanism between the texture and malignancy branches.",
    )
    ...
    model = MultiTaskCrossTalkNet(cross_talk=args.cross_talk).to(device)
    ...
    print(f"cross_talk: {args.cross_talk}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for images, tex_onehot, mal_labels in train_loader:
            images = images.to(device)
            tex_onehot = tex_onehot.to(device)
            mal_labels = mal_labels.to(device)
            ...
            tex_loss = one_hot_cross_entropy(tex_logits, tex_onehot)
            mal_loss = ce_malignancy(mal_logits, mal_labels)
            loss = args.texture_loss_weight * tex_loss + args.malignancy_loss_weight * mal_loss

            loss.backward()
            optimizer.step()

            running_loss += float(loss.item())
            texture_preds.append(torch.argmax(tex_logits, dim=1).detach().cpu())
            texture_labels.append(torch.argmax(tex_onehot, dim=1).detach().cpu())
            malignancy_preds.append(torch.argmax(mal_logits, dim=1).detach().cpu())
            malignancy_labels.append(mal_labels.detach().cpu())
            n_batches += 1

        train_metrics = {
            "loss": running_loss / max(1, n_batches),
            "texture": compute_classification_metrics(
                preds=torch.cat(texture_preds) if texture_preds else torch.empty(0, dtype=torch.long),
                labels=torch.cat(texture_labels) if texture_labels else torch.empty(0, dtype=torch.long),
                class_names=TEXTURE_CLASS_NAMES,
            ),
            "malignancy": compute_classification_metrics(
                preds=torch.cat(malignancy_preds) if malignancy_preds else torch.empty(0, dtype=torch.long),
                labels=torch.cat(malignancy_labels) if malignancy_labels else torch.empty(0, dtype=torch.long),
                class_names=MALIGNANCY_CLASS_NAMES,
            ),
        }
        val_metrics = evaluate(
            ...
            malignancy_loss_weight=args.malignancy_loss_weight,
            device=device,
        )
        ...
        print(
            ...
            f"{format_task_metrics('train_tex', train_metrics['texture'])} "
            f"{format_task_metrics('train_mal', train_metrics['malignancy'])} | "
            f"val_loss={val_metrics['loss']:.4f} "
            f"{format_task_metrics('val_tex', val_metrics['texture'])} "
            f"{format_task_metrics('val_mal', val_metrics['malignancy'])}"
        )
    ...
    print(
        ...
        f"{format_task_metrics('texture', test_metrics['texture'])} "
        f"{format_task_metrics('malignancy', test_metrics['malignancy'])}"
    )
    print_per_class_metrics("test", "texture", test_metrics["texture"])
    print_per_class_metrics("test", "malignancy", test_metrics["malignancy"])
